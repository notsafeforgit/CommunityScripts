...
class StashInterface:
    ...
    def __init__(self, fragment):
        ...
        if hook_ctx:
            ...
            self._content_id = hook_ctx.get("id")
            # Determine content type from hook type
            if "Scene" in self._hook_type:
                self._content_type = "scene"
            elif "Image" in self._hook_type:
                self._content_type = "image"
            elif "Gallery" in self._hook_type:
                self._content_type = "gallery"
            else:
                self._content_type = "scene"  # default fallback
        else:
            self._content_id = None
            self._content_type = "scene"
        self._path_rewrite = self._fragment["args"].get("pathRewrite")
        log.LogDebug(
            f"Starting nfoSceneParser plugin for {self._content_type} {self._content_id}")
    ...
    def get_scene_id(self):
        ...
        return self._content_id if self._content_type == "scene" else None
    ...
    def gql_findImage(self, image_id):
        ...
        result = self.__gql_call(query, variables)
        if (self._path_rewrite is not None):
            result["findImage"]["files"][0]["path"] = result["findImage"]["files"][0]["path"].replace(
                self._path_rewrite[0], self._path_rewrite[1])
        return result.get("findImage")

    def gql_findGallery(self, gallery_id):
        query = """
        ...
        result = self.__gql_call(query, variables)
        if (self._path_rewrite is not None):
            if result["findGallery"].get("folder") and result["findGallery"]["folder"].get("path"):
                result["findGallery"]["folder"]["path"] = result["findGallery"]["folder"]["path"].replace(
                    self._path_rewrite[0], self._path_rewrite[1])
            elif result["findGallery"].get("files") and len(result["findGallery"]["files"]) > 0:
                result["findGallery"]["files"][0]["path"] = result["findGallery"]["files"][0]["path"].replace(
                    self._path_rewrite[0], self._path_rewrite[1])
    ...
    def gql_findImages(self, tag_id=None):
        ...
        result = self.__gql_call(query, variables)
        return result.get("findImages")

    def gql_findGalleries(self, tag_id=None):
        query = """
        query FindGalleries($gallery_filter: GalleryFilterType, $filter: FindFilterType) {
    ...
    def gql_updateImage(self, image_id, image_data):
        ...
        input_data = {
            ...
            "performer_ids": image_data["performer_ids"],
            "tag_ids": image_data["tag_ids"],
        }
        # Update to "organized" according to config
        if config.set_organized_nfo and image_data["source"] == "nfo":
            has_mandatory_tags = True
            image_keys = [item[0].replace(
                "_id", "") if item[1] else None for item in image_data.items()]
            for mandatory_tag in config.set_organized_only_if:
                if mandatory_tag not in image_keys:
                    has_mandatory_tags = False
                    break
            if has_mandatory_tags:
                input_data.update({"organized": True})
    ...
    def gql_updateGallery(self, gallery_id, gallery_data):
        query = """
        ...
        input_data = {
            ...
            "details": gallery_data["details"],
            "date": gallery_data["date"],
            "rating100": gallery_data["rating"],
            "urls": gallery_data["urls"],
            "performer_ids": gallery_data["performer_ids"],
            "tag_ids": gallery_data["tag_ids"],
        }
        # Update to "organized" according to config
        if config.set_organized_nfo and gallery_data["source"] == "nfo":
            has_mandatory_tags = True
            gallery_keys = [item[0].replace(
                "_id", "") if item[1] else None for item in gallery_data.items()]
            for mandatory_tag in config.set_organized_only_if:
                if mandatory_tag not in gallery_keys:
                    has_mandatory_tags = False
                    break
            if has_mandatory_tags:
                input_data.update({"organized": True})
        ...
        result = self.__gql_call(query, variables)
        return result.get("galleryUpdate")
